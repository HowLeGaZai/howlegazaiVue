<template >
  <article
    class="chat-article"
    v-for="(chatTopic, index) in chatTopics"
    :key="index"
  >
    <section>
      <section class="chat_news">
        <div class="chat_info">
          <div :class="['tag', 'tag-mini', addClass(chatTopic.CATEGORY)]">
            {{ chatTopic.CATEGORY }}
          </div>
        ...
        <h2>
          <router-link
            :to="{ name: 'chat_info', params: { Id: chatTopic.ID } }"
            >{{ chatTopic.TITLE }}</router-link
          >
        </h2>
  ...
  mounted() {
    axios
      ...
      .then((response) => {
        ...
        this.chatTopics = data.slice(0, 5);
      })
      .catch((error) => {
        console.error(error);
      });
